...
function initializeGraph(
  ...
  attachBackgroundTap(cy, onBackgroundTap);
  attachCanvasFocus(cy, container);
 ...
 * a tap rather than the end of a pan. Panning while inspecting a node must not
 * clear the selection, or dismissing the highlight becomes its own trap.
 */
const BACKGROUND_TAP_TOLERANCE_PX = 6;

/**
 * Clicking empty canvas clears the selection — the affordance every graph tool
 * has and this one never had (no core-level tap handler existed at all, only
 * per-node ones). Guarded against pan-end by comparing press and release
 * positions rather than trusting the `tap` event to have excluded drags.
 */
function attachBackgroundTap(cy: cytoscape.Core, onBackgroundTap: () => void): void {
  let pressedAt: { x: number; y: number } | null = null;
  cy.on('mousedown', (event) => {
    pressedAt = event.target === cy ? event.renderedPosition : null;
  });
  cy.on('tap', (event) => {
    const releasedAt = event.renderedPosition;
    const origin = pressedAt;
    pressedAt = null;
    if (event.target !== cy || origin === null || releasedAt === undefined) {
      return;
    }
    if (Math.hypot(releasedAt.x - origin.x, releasedAt.y - origin.y) > BACKGROUND_TAP_TOLERANCE_PX) {
...
/**
 * Gives the graph container keyboard focus when the canvas is clicked.
 *
 * Without this, `Escape` was wired and unreachable: the handler lives on the
 * container's `onKeyDown`, Cytoscape's canvas consumes the mousedown, and
 * focus stayed on `<body>` — measured on the shipped binary as
 * `activeElement: BODY`. Esc therefore worked only for a user who arrived by
 * Tab, which is exactly the path an automated keyboard test takes, and never
 * for a user who clicked.
 */
function attachCanvasFocus(cy: cytoscape.Core, container: HTMLDivElement | null): void {
  if (container === null) {
    return;
  }
  cy.on('mousedown', () => container.focus({ preventScroll: true }));
}
...
export interface UseCytoscapeOptions {
  ...
  /** Tapping empty canvas — the conventional "clear it" gesture. */
...
export function useCytoscape(options: UseCytoscapeOptions): UseCytoscapeApi {
  const { containerRef, result, isReducedMotion, onNodeTap, onBackgroundTap } = options;
